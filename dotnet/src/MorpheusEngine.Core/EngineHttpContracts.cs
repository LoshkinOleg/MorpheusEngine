...
/// <summary>
/// JSON envelope for generic_llm_provider GET /debug/last_llm_payload.
/// Returns the most recent Ollama wire JSON from POST /chat or POST /generate, or <see cref="Available"/>=false if none yet.
/// </summary>
public sealed record LlmProviderLastPayloadResponse(
    [property: JsonPropertyName("ok")] bool Ok,
...
public static class EngineContractExamples
{
    ...
    public static EndpointTemplatePair? TryGetTemplates(string? templateContractsId) => templateContractsId switch
    {
        ...
        "llm_provider_last_llm_payload" => new EndpointTemplatePair(
            null,
            Serialize(new LlmProviderLastPayloadResponse(
                true,
                true,
                "chat",
                "2026-05-13T08:00:00Z",
                "{\"model\":\"qwen2.5:7b-instruct\",\"messages\":[{\"role\":\"system\",\"content\":\"You are the GM.\"}]}"))),
        _ => null
    };
