...
internal sealed class LlmProviderQwenHarness : IAsyncDisposable
{
    ...
    private LlmProviderQwenHarness(TempGameProject gameProject, EngineConfiguration configuration)
    {
        ...
        _lifecycle = new SingleListenerLifecycle(Client, _host.Run(), "LlmProviderQwen", ProviderPort);
    }
    ...
    {
        var collector = new HarnessTeardownErrorCollector(nameof(LlmProviderQwenHarness));
        await collector.RunAsync(
            "llm_provider_qwen.shutdown",
